...
export class Todos extends Component {
    ...
    DeleteTodo = (id) => {
        ...
        axios.get(`http://localhost:3030/DeleteTodo?id=${id}`).then(res => console.log(res)).then(() => {
            this.getTodos();
        });
    }
    ...
    addTodo = (Val) => {
        ...
        axios.get(`http://localhost:3030/AddTodo?todo="${Val}"&title="${this.props.todoTitles.Title}"`).then(res => console.log(res))
        .then(() => {
            this.getTodos();
        });
    }
    ...
    getTodos() {
        axios.get(`http://localhost:3030/GetTodos?title="${this.props.todoTitles.Title}"`)
